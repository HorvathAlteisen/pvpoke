<script lang="ts">
	/**
	 * modules/pokemultiselect.php — the group selector (`.poke.multi`) plus its modals, with
	 * a nested PokeSelect (PHP `require`, so it repeats for every instance).
	 *
	 * The PHP included cupselect.php with `require_once`, i.e. the cup dropdown and its
	 * `updateCupSelect()` script are emitted only ONCE per page even when several
	 * PokeMultiSelects are present. Pages therefore pass `cupSelect={false}` on every
	 * instance after the first (and on pages that render <CupSelect /> themselves).
	 * Independently of that, the PHP skipped cupselect entirely on the Team Builder — that
	 * check is done here from the request URI.
	 */
	import { page } from '$app/state';
	import CupSelect from './CupSelect.svelte';
	import Pokebox from './Pokebox.svelte';
	import PokeSelect from './PokeSelect.svelte';

	let { cupSelect = true }: { cupSelect?: boolean } = $props();

	const teamBuilder = $derived(page.data.requestUri.includes('team-builder'));
	const showCupSelect = $derived(cupSelect && !teamBuilder);
</script>
