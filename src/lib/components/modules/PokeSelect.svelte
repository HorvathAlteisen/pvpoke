<script lang="ts">
	/**
	 * modules/pokeselect.php — the single-Pokemon selector (`.poke.single`). Included twice
	 * by battle.php, once (hidden) by the pages that use PokeMultiSelect, and nested inside
	 * PokeMultiSelect itself. No props: the Editor's Score/Notes fields are driven by the
	 * request URI, exactly like the PHP `strpos($_SERVER['REQUEST_URI'], 'overrideEditor')`.
	 */
	import { page } from '$app/state';
	import Pokebox from './Pokebox.svelte';

	const { webRoot } = $derived(page.data);
	const overrideEditor = $derived(page.data.requestUri.includes('overrideEditor'));
</script>
...
<div class="poke single">
	...
	<div class="custom-move hide">
		...
		<p>You can customize moves in-depth with the <a href="{webRoot}gm-editor/">Gamemaster Editor.</a></p>
